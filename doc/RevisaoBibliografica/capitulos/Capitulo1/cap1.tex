...
\chapter{Introdução}
    O tamanho de dados utilizados cresce cada vez mais, a extração de informação em boa performance e a apresentação das mesmas são desafios cada vez maiores, além de que existe uma necessidade cada vez maior de aplicações que ofereçam uma interatividade com a visualização dos mesmos com respostas instantâneas aos comandos do usuário.
    
    São tantos detalhes e perspectivas para alcançar o objetivo de se utilizar os dados que para cumprir esta tarefa é necessário o conhecimento de diversas áreas, dentre elas Visualização de Dados, Mineração de Dados e Bases de Dados Não Relacionais, áreas que seram melhor esclarecidas nos capítulos que seguem.
    
\section{Objetivos do Trabalho}
    O objetivo deste trabalho é elucidar as técnicas presentes em áreas de Visualização de Dados, Mineração de Dados e Bases de Dados NOSQL a ponto de que seja possível acoplar estes conhecimentos a fim de que seja obtido um resultado satisfatório em análises de informações espaciais e temporais no uso da aplicação.
    
    Para isso é proposto a utilização de Bases de Dados não Relacionais junto com Técnicas de Visualização de dados procurando verificar a performance que pode ser obtida.

    \section{Organização do Trabalho}
    Este documento começa com uma pequena introdução deste capítulo, depois segue na fundamentação teórica dividida em:

    Bancos de Dados Não Relacionais: sendo abordados aspectos teóricos e de base da área.

    PostgreSQL e os tipos de dados não relacionais: aspectos sobre o próprio PostgreSQL e os novos tipos de dados orientados a documento que vem fornecendo

    Visualização de Dados: Novamente, aspectos gerais e de base da área envolvento `Descoberta do conhecimento' e `Mineração de Dados'.

    Por fim, os trabalhos relacionados a este assundo e a conclusão.

\chapter{Bancos de Dados Não Relacionais - NoSQL}
    Em um trabalho acadêmico intitulado "Selected Topics on Software-Technology Ultra-Large Scale Sites"(Tópicos Selecionados na Tecnologia de Softwares de Sites de Escala Ultra Grande) \cite{strauch} na Stuttgart Media University(Universidade de Mídia de Stuttgart) Christof Strauch reúne várias referências e tópicos sobre todo o assunto de base do NoSQL, conforme seu trabalho, os bancos de dados relacionais são os tipos predominantes para armazenamento de dados estruturados, desde 1970 este tipo de base de dados se suporta em uma aritmética relacional com o proliferado uso do SQL(Structured Query Language) para inserir, alterar, recuperar e remover os dados. Bases de dados não relacionais eram utilizadas apenas para aplicações de algum nicho específico como projetos utilizando SOAP que utiliza o padrão XML, porém o pensamento por cima desse uso disseminado "um tamanho para caber todos" tem sido questionado tanto pelas empresas como pelos acadêmicos devido ao aumento de usuários e volume de dados no meio.

	Conforme segue na argumentação de Christof Strauch, para suprir essa demanda algumas alternativas começam a ser propostas, vários outros modelos são criados para abrangir mais nichos antes dominados pelos SGBDRs, esses modeloss adotam caminhos não relacionais nem sempre garantindo a ACID (Atomicidade, Consistência, Isolamento e Durabilidade) dos dados, porém conseguem optimizar o espaço de armazenamento e a performance das bases de dados, não necessariamente se desfazendo do uso do SQL, por isso a sigla NoSQL significa $"not$ $only$ $structured$ $query$ $language"$.
...
\item Código Fonte aberto: Esse princípio assegura que o código fonte de bases NoSQL estejam disponíveis para que possam ser alterados para melhor se adaptar a necessidade de cada aplicação, diferentemente de SGBDRs comerciais que, além de serem muito onerosos, 
...
\chapter{PostgreSQL e os tipos de dados não relacionais}

\section{PostgreSQL}


%falta completar aquiiiiii
PostgreSQL\cite{postgresql} é um sistema gerenciador de banco de dados relacional robusto de reconhecimento em confiabilidade, integridade dos dados e correção. Tem seu código fonte aberto e seu desenvolvimento existe a mais de 15 anos. Disponível para diversos SOs, com interfaces nativas para muitas linguagens. Garante ACID e suporta a maioria dos tipos de dados SQL:2008. Tendo também o conjunto de funcionalidades dos SGBDs mais modernos como: triggers, chaves estrangeiras, views, joins...
...
\end{itemize}
	...
	Utiliza um gráfico paralelo aos eixos x e y onde cada atributo é um eixo paralelo e uma linha traçada perpendicularmente corta os eixos denotando o valor como na Figura~\ref{fig_geom}. Este tipo visa demonstrar bi ou tridimensionalmente visualizações de n dimensões 

	\begin{figure}[!htb]
    	\centering
    	\includegraphics[width=0.5\textwidth]{figuras/chapter1/geom}
    	\caption{Correlação e distribuição de valores em Coordenadas Paralelas. \apud{war1994}{shimabukuro}.}
    	\label{fig_geom}
  	\end{figure}
	
	O problema desta técnica é que a imagem pode ficar saturada com o número de retas inseridas como na Figura~\ref{fig_geom_sat}
	
	\begin{figure}[!htb]
    	\centering
    	\includegraphics[width=0.5\textwidth]{figuras/chapter1/geom_sat}
    	\caption{Sobreposições de linhas com apresentação de 473 itens. \apud{ins1997}{shimabukuro}.}
    	\label{fig_geom_sat}
  	\end{figure}

	\subsection{Técnicas Iconográficas}
	Nesta técnica cada item dos dados é transposto a um ícone, como na técnica de `Figura de Arestas'(Stick Figure) em que cada dimensão da tela é um atributo dos dados, no caso 2, e os outros atributos podem ser o ângulo de cada aresta e(ou) o comprimento da mesma, como na Figura~\ref{fig_icon}. Outras características podem ser disponibilizadas para os atributos como a cor e a expessura das arestas, O conjunto de vários itens de dados também pode gerar um padrão de textura de rápida percepção visual como na Figura~\ref{fig_text}. A desvantagem desta técnica é poder apresentar um número máximo de dimensões perto de 10.

	\begin{figure}[!htb]
    	\centering
    	\includegraphics[width=0.5\textwidth]{figuras/chapter1/icon}
    	\caption{Configuração para Figura de Arestas usando cinco arestas \apud{kei1996}{shimabukuro}.}
    	\label{fig_icon}
  	\end{figure}

	\begin{figure}[!htb]
    	\centering
    	\includegraphics[width=0.5\textwidth]{figuras/chapter1/text}
    	\caption{Diferentes texturas representadas por Figura de Arestas. \apud{gri2001}{shimabukuro}.}
    	\label{fig_text}
  	\end{figure}

	\subsection{Técnicas Hierárquicas}

	Nas técnicas de visualização hierárquicas, a janela é dividida em um conjunto de espaços e cada espaço pode ter uma subdivisão dentro de si em mais espaços representando atributos para cada divisão como na Figura~\ref{fig_hier}. essa técnica também é conhecida como `Empilhamento Dimensional'(Dimensional Stacking). A dificuldade é que não é possível levantar muitos atributos por causa das subdivisões no espaço da tela, a não ser que exista alguma funcionalidade interativa para ampliar e reduzir o tamanho da imagem.

	\begin{figure}[!htb]
    	\centering
    	\includegraphics[width=0.5\textwidth]{figuras/chapter1/hier}
    	\caption{Resultado do empilhamento dimensional. Quatro atributos são aninhados aos pares \apud{hoff1977}{shimabukuro}.}
    	\label{fig_hier}
  	\end{figure}

	\subsection{Variação Temporal Uni-escala}
    Shimabukuro\cite{shimabukuro} propõe um novo modelo que se encaixa muito bem para atributos que apresentam uma variação temporal em alguma escala definida(dia, semana, ano, ...) utilizando uma tabela de cores, mapeando os intervalos de valores em interesse nas mesmas. Tais valores podem ser representados por sequências de pixels ou pontos organizados como quadrados como na Figura~\ref{fig_uni}. O autor descreve que cada sequência de pixels é correspondente a um objeto espacial, colocados verticalmente ou horizontalmente. Esta técnica permite uma visão simultânea de vários objetos referentes ao seu valor durante o intervalo de tempo.

	\begin{figure}[!htb]
    	\centering
    	\includegraphics[width=0.5\textwidth]{figuras/chapter1/uni}
    	\caption{Construção da representação visual `Variação Temporal Uni-escala'.\cite{shimabukuro}.}
    	\label{fig_uni}
  	\end{figure}

	\subsection{Variação Temporal Multi-escala}
    A técnica multi-escala é similar a uni-escala porém, ao invés de ter o foco em observar vários objetos ao longo do tempo, ela tem o foco de observar apenas um objeto com um detalhamento temporal mais específico. Neste caso são agrupados os valores do objeto em diferentes escalas de tempo na mesma janela, tomando um caso qualquer seria possível observar os valores diários, mensais, anuais e no decorrer dos anos de tal objeto como na Figura~\ref{fig_multi}.

    \begin{figure}[!htb]
    	\centering
    	\includegraphics[width=0.5\textwidth]{figuras/chapter1/multi}
    	\caption{Construção da representação visual `Variação Temporal Multi-escala'.\cite{shimabukuro}.}
    	\label{fig_multi}
  	\end{figure}

\chapter{Trabalhos Relacionados}
    Os trabalhos relacionados estão todos como parte base para esta revisão, sendo eles:

\begin{itemize}

    \item \cite{genghini}: Genghini implementa alguns métodos de visualização de \cite{shimabukuro} com interatividade que também serão posteriormente implementados no decorrer do TCC.
    \item \cite{heer}: Implementa a biblioteca D3 fornecendo ferramentas de suporte para a visualização de dados, que será até mesmo a tecnologia base para a implementação das visualizações deste projeto.
    \item \cite{whittaker}: Implementou os campos Json e Jsonb no PostgreSQL através do source do mesmo e de bibliotecas do CouchDB

\end{itemize}

\chapter{Conclusões}
    A quantidade crescente de dados tem impulsionado estudos de diversas áreas para conseguirmos obter a informação através de técnicas de mineração de dados, interpretar a informação através de técnicas de visualização de dados e fazer isso de forma fluida, com uma boa performance, e para isso é necessário avaliar de que forma o computador processará tudo, nesse passo, podemos usufruir de Bases NOSQL.
